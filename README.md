...
- 自动识别漏掉开局时，可在悬浮窗点击“手动截取开始盘面”立即开始记录。
- 保存开始与结束两张盘面，并以间隔和重复区域提示进行拼接。
- 保存 K 线、成交量、大单净量区域以及结果卡片。
- 直接读取训练营窗口图像，其他窗口遮挡训练营时不会被录入复盘截图。
- 自动生成包含股票、代码、训练区间、收益率、彩色标签和图片的 Obsidian Markdown 笔记。
- 内置本地数据看板，展示训练局数、平均收益、胜率、最佳单局和收益趋势。
- 支持按时间、股票、标签和结果筛选历史记录，并查看每局完整盘面与结果卡片。
- 可在训练详情中删除记录，并同步清理未被其他笔记引用的截图。
- 可在悬浮窗中创建自定义名称和颜色的标签预设，并给当前或下一局训练添加多个标签。
- 悬浮窗显示当前识别、截图和保存进度。
- 支持窗口缩放、不同 DPI 和多显示器。
...
悬浮窗底部点击“标签”可以选择已有标签，也可以创建自定义名称和颜色的标签。选中的标签会写入本局笔记，保存完成后自动清空；标签预设会保留供下次使用。数据看板可直接搜索标签，也可在历史记录上方按标签筛选。
...
1. 识别到训练盘面的 `30/30`。
2. 确认顶部行情、K 线和均线完整且画面稳定后保存开始盘面；识别失败时可手动开始。
3. 最后一根 K 线出现后保存结束盘面。
